...
export function BCContaCorrente() {
  ...
  } = useBusinessCentralCustomers(search);
  ...
  const totalBalance =
    ...
    ledgerEntries.filter((e) => e.is_open).reduce((sum, e) => sum + (e.remaining_amount || 0), 0);
      ...
      <Card>
        ...
        <CardContent>
          <Alert>
            ...
            <AlertTitle>
              {t('businessCentral.notConfigured.title', 'Integração não configurada')}
            </AlertTitle>
            <AlertDescription>
              {t(
      ...
      <CardContent className="space-y-4">
        ...
        <div className="space-y-2">
          <div className="flex gap-2">
            <div className="relative flex-1">
              ...
              <Input
                placeholder={t(
                  'businessCentral.searchCustomer',
                  'Pesquisar cliente por nome, número ou NIF...',
                )}
                value={search}
                onChange={(e) => {
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">
                          {customer.bc_number && (
                            <span className="mr-2">#{customer.bc_number}</span>
                          )}
                      ...
                      {customer.balance !== null && (
                        <p
                          className={`text-sm font-semibold ${customer.balance > 0 ? 'text-destructive' : 'text-green-600'}`}
                        >
          ...
          <>
            <div className="rounded-lg border bg-muted/30 p-4">
              <div className="flex items-start justify-between">
                <div className="space-y-1">
                  ...
                  <div className="flex flex-wrap gap-x-4 gap-y-1 text-sm text-muted-foreground">
                    {selectedCustomer.bc_number && (
                      <span>
                        {t('businessCentral.customer.number', 'Nº Cliente')}:{' '}
                        {selectedCustomer.bc_number}
                      </span>
                    )}
                    {selectedCustomer.nif && <span>NIF: {selectedCustomer.nif}</span>}
                ...
                <div className="text-right space-y-1">
                  <p className="text-xs text-muted-foreground">
                    {t('businessCentral.customer.openBalance', 'Saldo em aberto')}
                  </p>
                  <p
                    className={`text-xl font-bold ${totalBalance > 0 ? 'text-destructive' : 'text-green-600'}`}
                  >
                    {formatCurrency(totalBalance, selectedCustomer.currency_code || 'EUR')}
                  </p>
                  {overdueBalance > 0 && (
                    <p className="text-xs text-destructive">
                      {t('businessCentral.customer.overdueBalance', 'Vencido')}:{' '}
                      {formatCurrency(overdueBalance, selectedCustomer.currency_code || 'EUR')}
                    </p>
                  )}
                </div>
              </div>
              ...
              <div className="rounded-md border overflow-x-auto">
                <Table>
                  <TableHeader>
                    <TableRow>
                      <TableHead>
                        {t('businessCentral.ledger.postingDate', 'Data Lançamento')}
                      </TableHead>
                      ...
                      <TableHead className="text-right">
                        {t('businessCentral.ledger.amount', 'Montante')}
                      </TableHead>
                      <TableHead className="text-right">
                        {t('businessCentral.ledger.remainingAmount', 'Montante Restante')}
                      </TableHead>
                  ...
                  <TableBody>
                    {ledgerEntries.map((entry) => (
                      <TableRow key={entry.id}>
                        ...
                        <TableCell
                          className={`text-right text-sm font-semibold ${(entry.amount || 0) < 0 ? 'text-green-600' : ''}`}
                        >
            ...
            {ledgerEntries.length > 0 && (
              <div className="flex justify-between items-center text-sm text-muted-foreground pt-2 border-t">
                <span>
                  {t('businessCentral.ledger.count', '{{count}} lançamento(s)', {
                    count: ledgerEntries.length,
                  })}
                </span>
                {ledgerFilter !== 'closed' && (
                  <span>
                    {t('businessCentral.ledger.totalOpen', 'Total em aberto')}:{' '}
        ...
        {!selectedCustomer && !isLoadingCustomers && (
          <div className="text-center py-6 text-muted-foreground text-sm">
            ...
            <p>
              {t(
                'businessCentral.searchHint',
                'Pesquise e selecione um cliente para ver a sua conta corrente',
              )}
            </p>
          </div>
        )}
