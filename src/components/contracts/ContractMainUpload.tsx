...
export function ContractMainUpload({
  contratoId,
  storagePath: _storagePath,
}: ContractMainUploadProps) {
  ...
  const handleUpload = async (file: File) => {
    ...
    try {
      ...
      const uploadedPath = result?.url_ficheiro ?? _storagePath ?? '';

      await supabase
        .from('contratos')
        .update({ validation_status: 'validating' } as unknown as never)
        .eq('id', contratoId);
