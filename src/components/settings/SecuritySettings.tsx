...
export function SecuritySettings() {
  ...
  const { profile, isLoading: _isLoadingProfile, updateProfile } = useProfile();
  ...
  const handleEnable2FA = async () => {
    ...
    try {
      ...
      toast.error(
        error instanceof Error ? error.message : 'Erro ao ativar autenticação de dois fatores.',
      );
    } finally {
  ...
  const handleDisable2FA = async () => {
    try {
      ...
      toast.error(
        error instanceof Error ? error.message : 'Erro ao desativar autenticação de dois fatores.',
      );
    }
  };
        ...
        <CardContent className="space-y-4">
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  {t(
                    'security.twoFactor.authenticatorDesc',
                    'Use uma aplicação como Google Authenticator ou Authy.',
                  )}
                </p>
              </div>
            </div>
          ...
          <div className="rounded-lg bg-muted/50 p-4 space-y-2">
            ...
            <ul className="text-sm text-muted-foreground list-disc list-inside space-y-1">
              <li>
                {t(
                  'security.recommendations.strongPassword',
                  'Use uma palavra-passe forte e única',
                )}
              </li>
              <li>
                {t('security.recommendations.enable2FA', 'Ative a autenticação de dois fatores')}
              </li>
              <li>
                {t(
                  'security.recommendations.checkSessions',
                  'Verifique regularmente as sessões ativas',
                )}
              </li>
            </ul>
          </div>
        ...
        <AlertDialogContent className="max-w-md">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="space-y-4">
              <p>
                {t(
                  'security.twoFactor.setupStep1',
                  '1. Instale uma aplicação autenticadora (Google Authenticator, Authy, etc.)',
                )}
              </p>
              <p>
                {t(
                  'security.twoFactor.setupStep2',
                  '2. Digitalize o código QR ou introduza a chave manualmente:',
                )}
              </p>
              ...
              <p>
                {t(
                  'security.twoFactor.setupStep3',
                  '3. Introduza o código de 6 dígitos para verificar:',
                )}
              </p>
            ...
            <div className="flex items-center justify-between">
              <div>
                <p className="font-medium">
                  {t('security.sessions.currentSession', 'Sessão Atual')}
                </p>
                <p className="text-sm text-muted-foreground">
                  {t('security.sessions.browser', 'Navegador')}:{' '}
