  ...
  return (
    <div className="space-y-6">
      ...
      <Alert>
        ...
        <AlertTitle>
          {t('businessCentral.settings.infoTitle', 'Integração Business Central')}
        </AlertTitle>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            {t(
              'businessCentral.settings.configDesc',
              'Defina o URL do servidor BC e o GUID da empresa para esta organização.',
            )}
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          ...
          <div className="space-y-2">
            <Label htmlFor="bcUrl">
              {t('businessCentral.settings.bcUrl', 'URL do Servidor BC')}
            </Label>
            <Input
              id="bcUrl"
              value={bcUrl}
          ...
          {/* Company GUID */}
          <div className="space-y-2">
            <Label htmlFor="companyGuid">
              {t('businessCentral.settings.companyGuid', 'GUID da Empresa')}
            </Label>
            <Input
              id="companyGuid"
              value={companyGuid}
          ...
          <div className="space-y-2">
            <Label htmlFor="companyName">
              {t('businessCentral.settings.companyName', 'Nome da Empresa (opcional)')}
            </Label>
            <Input
              id="companyName"
              value={companyName}
            ...
            <div className="rounded-lg border bg-muted/50 p-4">
              <div className="flex items-center gap-2 text-sm">
                ...
                <span className="font-medium">
                  {t('businessCentral.settings.connected', 'Ligado a:')}
                </span>
          ...
          <div className="flex items-center justify-between rounded-lg border p-4">
            <div className="space-y-0.5">
              ...
              <p className="text-sm text-muted-foreground">
                {t(
                  'businessCentral.settings.enabledDesc',
                  'Ativar ou desativar a visualização dos dados BC nesta organização',
                )}
              </p>
            </div>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
                ...
                <div className="space-y-1">
                  <div className="flex items-center gap-2">
                    <span className="font-medium text-sm">
                      {t('businessCentral.settings.syncStatus', 'Estado:')}
                    </span>
              ...
              {syncStatus?.counts && (
                <div className="grid grid-cols-3 gap-4 pt-2 border-t">
                  <div className="text-center">
                    ...
                    <p className="text-xs text-muted-foreground">
                      {t('businessCentral.settings.customers', 'Clientes')}
                    </p>
                  </div>
                  <div className="text-center">
                    <p className="text-2xl font-bold">{syncStatus.counts.accounts}</p>
                    <p className="text-xs text-muted-foreground">
                      {t('businessCentral.settings.accounts', 'Contas')}
                    </p>
                  </div>
                  <div className="text-center">
                    <p className="text-2xl font-bold">{syncStatus.counts.ledger_entries}</p>
                    <p className="text-xs text-muted-foreground">
                      {t('businessCentral.settings.ledgerEntries', 'Lançamentos')}
                    </p>
                  </div>
                </div>
              )}
            ...
            <Alert>
              <Info className="h-4 w-4" />
              <AlertTitle>
                {t('businessCentral.settings.agentTitle', 'Agente de sincronização local')}
              </AlertTitle>
              <AlertDescription>
                <p className="text-sm">
                  {t(
            ...
            {syncStatus?.logs && syncStatus.logs.length > 0 && (
              <div className="space-y-2">
                <h5 className="text-sm font-medium">
                  {t('businessCentral.settings.recentLogs', 'Histórico recente')}
                </h5>
                <div className="space-y-2">
                  {syncStatus.logs.map((log) => (
                    ...
                    >
                      ...
                      <div className="text-right text-xs text-muted-foreground">
                        {log.status === 'success' && (
                          <span>
                            {log.customers_synced} clientes · {log.accounts_synced} contas ·{' '}
                            {log.ledger_entries_synced} lançamentos
                          </span>
                        )}
            ...
            <div className="flex items-center justify-between">
              <div>
                <p className="font-medium">
                  {t(
                    'businessCentral.settings.removeIntegration',
                    'Remover integração Business Central',
                  )}
                </p>
                <p className="text-sm text-muted-foreground">
                  {t(
