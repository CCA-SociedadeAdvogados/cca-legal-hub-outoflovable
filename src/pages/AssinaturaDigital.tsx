...
export default function Documentos() {
  ...
  const {
    documentos,
    isLoading,
    createDocumento: _createDocumento,
    deleteDocumento,
    sendForSignature,
  } = useDocumentosGerados({ modulo: 'ASSINATURA' });
              ...
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm text-muted-foreground">
                    {t('documents.signatureStatus.pending')}
                  </p>
          ...
          <Card>
            <CardContent className="pt-6">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm text-muted-foreground">
                    {t('documents.signatureStatus.sent')}
                  </p>
          ...
          <Card>
            <CardContent className="pt-6">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm text-muted-foreground">
                    {t('documents.signatureStatus.signed')}
                  </p>
            ...
            <form onSubmit={handleSubmitSignature} className="space-y-4">
              <div className="p-3 bg-muted/50 rounded-lg">
                ...
                <p className="text-xs text-muted-foreground">
                  {formatFileSize(selectedDocumento?.tamanho_bytes || null)}
                </p>
              </div>
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(v) =>
                    setSignatureData({ ...signatureData, prazo_dias: parseInt(v) })
                  }
                >
                  <SelectTrigger>
                    <SelectValue />
            ...
            {filteredDocumentos.map((documento) => (
              <Card key={documento.id}>
                <CardHeader className="pb-2">
                  <div className="flex items-start justify-between">
                    ...
                    <Badge className={estadoAssinaturaColors[documento.estado_assinatura]}>
                      ...
                      <span className="ml-1">
                        {getEstadoAssinaturaLabel(documento.estado_assinatura)}
                      </span>
                    </Badge>
                  </div>
                </CardHeader>
                <CardContent>
                  ...
                  <div className="flex items-center justify-between">
                    <p className="text-sm text-muted-foreground">
                      {t('documents.createdOn')}{' '}
                      {new Date(documento.created_at).toLocaleDateString('pt-PT')}
                    </p>
                    <div className="flex gap-2">
                      ...
                      {documento.estado_assinatura === 'pendente' && (
                        <Button
                          variant="ghost"
                          size="sm"
                          onClick={() => handleSendForSignature(documento)}
                        >
