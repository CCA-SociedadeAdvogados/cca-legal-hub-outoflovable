...
function getNotificationIcon(type: string) {
  if (type.startsWith('contract_expiry')) return <FileText className="h-4 w-4 text-warn" />;
  if (type === 'news_published') return <Newspaper className="h-4 w-4 text-brand" />;
  return <Bell className="h-4 w-4 text-ink-mute" />;
}

/**
 * PortalHeader — topbar enxuto do Portal do Cliente.
 * Sem org switcher nem atalhos internos da CCA; tudo aponta para rotas /portal.
 */
export function PortalHeader() {
  ...
  const handleNotificationClick = (notification: {
    ...
    if (notification.reference_type === 'cca_news') {
      navigate('/portal/novidades');
    } else if (notification.reference_type === 'contratos' && notification.reference_id) {
      navigate(`/portal/contratos/${notification.reference_id}`);
    }
  };
        ...
        <DropdownMenu>
          ...
          >
            ...
            <DropdownMenuItem asChild className="cursor-pointer">
              <Link to="/portal/novidades" className="flex w-full items-center justify-between">
