-- ============================================================
-- Migration: corrigir legacy_client_name na vw_cca_client_catalog_overview
--
-- Problema: para clientes cujo organizations_legacy.name foi populado
-- com o próprio client_code (em vez do nome real da empresa),
-- a view devolvia o código duas vezes — como nome e como código.
--
-- Solução: NULLIF(ol.name, ol.client_code) → devolve NULL quando
-- o "nome" é idêntico ao código, permitindo ao frontend usar o código
-- como fallback sem mostrar informação redundante.
-- ============================================================
...
SELECT
    ol.client_code,
    -- Quando name = client_code o campo não tem valor real → devolver NULL
    -- O frontend usa client_code como fallback quando este campo é NULL
    NULLIF(ol.name, ol.client_code) AS legacy_client_name,
