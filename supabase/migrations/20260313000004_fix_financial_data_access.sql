-- ============================================================
-- Migration: corrigir acesso aos dados financeiros para CCA e clientes
--
-- Contexto: as funções financeiras e a view de catálogo não
-- conseguiam aceder à tabela organizations para clientes não
-- membros devido ao RLS restritivo existente.
--
-- Fix A: fn_is_cca_internal_authorized → SECURITY DEFINER
--        (evita recursão infinita quando usada em políticas RLS
--         da mesma tabela organizations)
--
-- Fix B: nova política RLS em organizations para CCA interno
--        (desbloqueia vw_cca_client_catalog_overview e searchCCAClients)
--
-- Fix C: fn_get_financial_* → SECURITY DEFINER
--        (a lógica de autorização interna (CCA vê tudo, owner vê
--         grupo, outros vêem a própria entidade) passa a funcionar
--         correctamente sem ser bloqueada pelo RLS do chamador)
-- ============================================================
...
-- ── Fix B: política RLS para CCA interno ver todas as organizations ─

DO $$ BEGIN
  CREATE POLICY "CCA internal can view all organizations"
    ON public.organizations FOR SELECT
    USING (public.fn_is_cca_internal_authorized(auth.uid()));
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;

-- ── Fix C: fn_get_client_home_for_actor → SECURITY DEFINER ────────
...
-- ── Fix C: fn_get_financial_items_for_actor → SECURITY DEFINER ────
...
-- ── Fix C: fn_get_financial_summary_for_actor → SECURITY DEFINER ──
...
-- ── Fix C: fn_get_financial_summary_by_entity_for_actor → SECURITY DEFINER
