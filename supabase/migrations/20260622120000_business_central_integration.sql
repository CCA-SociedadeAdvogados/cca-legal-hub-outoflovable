...
-- via agente local de sincronização (scripts/bc-sync-agent).
--
-- RLS adaptado às convenções actuais:
--   - leitura: membros da org (get_user_organization_id) ou platform admin
--   - gestão de bc_config: platform admin
--   - escrita de dados sincronizados: service_role (agente local)
-- =====================================================
...
CREATE OR REPLACE FUNCTION public.update_bc_updated_at()
RETURNS TRIGGER AS $$
BEGIN
    NEW.updated_at = now();
    RETURN NEW;
...
DROP TRIGGER IF EXISTS trigger_bc_config_updated_at ON public.bc_config;
CREATE TRIGGER trigger_bc_config_updated_at
    BEFORE UPDATE ON public.bc_config
    FOR EACH ROW
    EXECUTE FUNCTION public.update_bc_updated_at();

DROP TRIGGER IF EXISTS trigger_bc_customers_updated_at ON public.bc_customers;
CREATE TRIGGER trigger_bc_customers_updated_at
    BEFORE UPDATE ON public.bc_customers
    FOR EACH ROW
    EXECUTE FUNCTION public.update_bc_updated_at();

DROP TRIGGER IF EXISTS trigger_bc_accounts_updated_at ON public.bc_accounts;
CREATE TRIGGER trigger_bc_accounts_updated_at
    BEFORE UPDATE ON public.bc_accounts
    FOR EACH ROW
    EXECUTE FUNCTION public.update_bc_updated_at();

DROP TRIGGER IF EXISTS trigger_bc_ledger_updated_at ON public.bc_ledger;
...
-- =====================================================
-- RLS Policies (convenções actuais)
-- =====================================================
...
-- ── bc_config ───────────────────────────────────────
DROP POLICY IF EXISTS "Org members can view bc config" ON public.bc_config;
...
CREATE POLICY "Platform admins can manage bc config"
    ON public.bc_config FOR ALL
    USING (public.is_platform_admin())
    WITH CHECK (public.is_platform_admin());

DROP POLICY IF EXISTS "Service role can manage bc config" ON public.bc_config;
CREATE POLICY "Service role can manage bc config"
    ON public.bc_config FOR ALL
    USING (auth.role() = 'service_role')
    WITH CHECK (auth.role() = 'service_role');

-- ── bc_customers ────────────────────────────────────
DROP POLICY IF EXISTS "Org members can view bc customers" ON public.bc_customers;
...
DROP POLICY IF EXISTS "Service role can manage bc customers" ON public.bc_customers;
CREATE POLICY "Service role can manage bc customers"
    ON public.bc_customers FOR ALL
    USING (auth.role() = 'service_role')
    WITH CHECK (auth.role() = 'service_role');

-- ── bc_accounts ─────────────────────────────────────
DROP POLICY IF EXISTS "Org members can view bc accounts" ON public.bc_accounts;
...
DROP POLICY IF EXISTS "Service role can manage bc accounts" ON public.bc_accounts;
CREATE POLICY "Service role can manage bc accounts"
    ON public.bc_accounts FOR ALL
    USING (auth.role() = 'service_role')
    WITH CHECK (auth.role() = 'service_role');

-- ── bc_ledger ───────────────────────────────────────
DROP POLICY IF EXISTS "Org members can view bc ledger" ON public.bc_ledger;
...
DROP POLICY IF EXISTS "Service role can manage bc ledger" ON public.bc_ledger;
CREATE POLICY "Service role can manage bc ledger"
    ON public.bc_ledger FOR ALL
    USING (auth.role() = 'service_role')
    WITH CHECK (auth.role() = 'service_role');

-- ── bc_sync_logs ────────────────────────────────────
DROP POLICY IF EXISTS "Org members can view bc sync logs" ON public.bc_sync_logs;
...
DROP POLICY IF EXISTS "Service role can manage bc sync logs" ON public.bc_sync_logs;
CREATE POLICY "Service role can manage bc sync logs"
