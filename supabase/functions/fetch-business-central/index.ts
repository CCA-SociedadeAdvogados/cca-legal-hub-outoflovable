...
/**
 * fetch-business-central
 *
 * Leitura (e gestão da configuração) dos dados Business Central sincronizados
 * em cache nas tabelas bc_*. Os dados são escritos pelo agente local
 * (scripts/bc-sync-agent) via service role; esta função serve o frontend
 * autenticado.
 *
 * Autorização: isAuthorizedForOrg garante que o chamador pertence à
 * organização (ou é platform admin / utilizador CCA SSO / service role).
 */
