...
async function ensureDemoTenant(
  // Note: Edge Functions don't have DB types available here; SupabaseClient defaults
  // to a generic schema to avoid `never` inference on table rows.
  ...
  if (oldMembershipsError) {
    ...
    const nonDemoMemberships = (oldMemberships as Array<{ id: string; organization_id: string; organizations?: { slug?: string } }>).filter(
